...
module Compiler = struct
    ...
    let objectExpr doc namePatt asExpr auditors meths matchs span =
        State.bind asExpr (fun ase ->
            State.bind (sequence auditors) (fun auds ->
                (* XXX rebind into env *)
                (fun s -> ((object (self)
                    ...
                    method call verb args namedArgs: monte option =
                      ...
                      | Some (_, params, nParams, body) ->
                         ...
                         let env' = List.fold_left2 (fun ma p s -> State.and_then ma (p s exit)) (State.return ()) params args
                         in Printf.printf "executing %s" verb;
                         let (o, _) = State.and_then env' body s in Some o
                    (* XXX miranda methods *)
                    (* XXX call printOn *)
